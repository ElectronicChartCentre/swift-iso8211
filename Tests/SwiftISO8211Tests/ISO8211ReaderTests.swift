...
struct ISO8211ReaderTests {
  ...
  @Test func testS57() async throws {
    ...
    guard
      let testDataURL = Bundle.module.url(
        forResource: "TestResources/US4CN21M", withExtension: "000")
    else {
  ...
  @Test func testS101() async throws {
    ...
    guard
      let testDataURL = Bundle.module.url(
        forResource: "TestResources/101AA00DS0003", withExtension: "000")
    else {
      Issue.record("Could not load test data")
      return
    }
    ...
    #expect(
      rec1!.fieldNodes(withTag: "DSID").first?.valueByLabel["DSNM"] as? String
        == "101AA00DS0003.000")
  ...
  @Test func testParseCASeaTrialsS101Data() async throws {
    ...
    if !fileManager.fileExists(atPath: localZipFilePath) {
      ...
      guard
        let zipURL = URL(
          string:
            "https://www.charts.gc.ca/documents/data-gestion/Unencrypted_S100_DatasetsNov2025.zip")
      else {
    ...
    for (fileName, data) in fileDataByName {
      ...
      while reader.hasMore() {
        guard let record = DataRecord.create(reader: reader, ddr: ddr) else {
          Issue.record(
            "Could not parse \(fileName) as a ISO8211 file - record \(dataRecords.count + 1)")
          return
        }
        dataRecords.append(record)
      }
