...
public struct DataDescriptiveRecord {
  ...
  init(
    leader: LogicalRecordLeader, directory: LogicalRecordDirectory,
    arrayDescriptorss: [ArrayDescriptors], formatControlss: [FormatControls]
  ) {
    self.leader = leader
    ...
    for (index, directoryEntry) in directory.entries.enumerated() {
      ...
      arrayDescriptorsFormatControlsByTag[directoryEntry.fieldTag] = (
        arrayDescriptors, formatControls
      )
    }
  ...
  public static func create(reader: BinaryReader) -> DataDescriptiveRecord? {
    ...
    for entry in directory.entries {
      ...
      // field controls
      guard reader.readString(numBytes: fieldControlLength) != nil else {
        return nil
      }

      // data field name
      guard reader.readStringUT(maxPos: fieldEnd) != nil else {
        return nil
      }
    ...
    return DataDescriptiveRecord(
      leader: leader, directory: directory, arrayDescriptorss: arrayDescriptorss,
      formatControlss: formatControlss)
  }
