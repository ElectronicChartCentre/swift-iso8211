...
public struct DataRecord {
  ...
  public static func create(reader: BinaryReader, ddr: DataDescriptiveRecord) -> DataRecord? {
    ...
    for directoryEntry in directory.entries {
      ...
      guard let arrayDescriptorsFormatControlsPair = ddr.arrayDescriptorsFormatControlsByTag[tag]
      else {
        return nil
      }
      ...
      if arrayDescriptors.labels.count != formatControls.formatControls.count {
        print(
          "ERROR: tag \(tag). array descriptors count: \(arrayDescriptors.labels.count) does not match format controls count: \(formatControls.formatControls.count)"
        )
        return nil
      }
      ...
      if hasRepeat {
        while (reader.pos() + 1) < fieldEndPos {
          ...
          let fieldNode = FieldNode(
            fieldTag: tag, valueByLabel: repeatFieldValueByLabel, children: [])
      ...
      if !repeatsFieldValueByLabel.isEmpty, arrayDescriptors.repetitionIndex ?? 0 > 0 {
        let fieldNode = FieldNode(
          fieldTag: tag, valueByLabel: fieldValueByLabel, children: repeatsFieldValueByLabel)
    ...
    if reader.peekByte() == Terminator.field {
      ...
      // not good.
      print(
        "ERROR: record end, but not at FT. reader at \(reader.pos()). expected \(String(describing: recordEndPos))"
      )
